...
export async function downloadPersonalExpensesPDF(
  txns: Transaction[],
  categories: Category[],
  period: Period,
  userName: string,
  ...
  doc.text("Personal Expense Report", 30, 18);
  ...
  doc.text("Type: Personal Expenses Only", W - 14, y + 10, { align: "right" });
  ...
  autoTable(doc, {
    ...
    didDrawPage: (data) => {
      ...
      doc.setTextColor(...GRAY_400);
      doc.setFont("helvetica", "normal");
      doc.text("Generated by The Ledger · Personal Expense Report", 14, pageH - 7);
  ...
  // ── Save ─────────────────────────────────────────────────────
  const fileName = `personal-expenses-${periodLabel.toLowerCase().replace(/\s+/g, "-")}-${format(new Date(), "yyyy-MM-dd")}.pdf`;
  doc.save(fileName);
}
